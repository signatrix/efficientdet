...
class BiFPN(nn.Module):
    ...
    def forward(self, inputs):
        ...
        p6_up = self.conv6_up(w1[0] * p6_in + w1[1] * self.p6_upsample(p7_up))
        # Connections for P5_0 and P6_0 to P5_1 respectively
        p5_up = self.conv5_up(w1[0] * p5_in + w1[1] * self.p5_upsample(p6_up))
        # Connections for P4_0 and P5_0 to P4_1 respectively
        p4_up = self.conv4_up(w1[0] * p4_in + w1[1] * self.p4_upsample(p5_up))
        # Connections for P3_0 and P4_1 to P3_2 respectively
        p3_out = self.conv3_up(w1[0] * p3_in + w1[1] * self.p3_upsample(p4_up))

        # Connections for P4_0, P4_1 and P3_2 to P4_2 respectively
        p4_out = self.conv4_down(w2[0] * p4_in + w2[1] * p4_up + w2[2] * self.p4_downsample(p3_out))
        # Connections for P5_0, P5_1 and P4_2 to P5_2 respectively
        p5_out = self.conv5_down(w2[0] * p5_in + w2[1] * p5_up + w2[2] * self.p5_downsample(p4_out))
        # Connections for P6_0, P6_1 and P5_2 to P6_2 respectively
        p6_out = self.conv6_down(w2[0] * p6_in + w2[1] * p6_up + w2[2] * self.p6_downsample(p5_out))
        # Connections for P7_0 and P6_2 to P7_2
        p7_out = self.conv7_down(w2[0] * p7_in + w2[1] * p7_up + w2[2] * self.p7_downsample(p6_out))
...
class EfficientNet(nn.Module):
    ...
    def forward(self, inputs):
        inputs = self.model.conv_stem(inputs)
        inputs = self.model.bn1(inputs)
        inputs = self.model.act1(inputs)
        feature_maps = []
...
class EfficientDet(nn.Module):
    def __init__(self, num_anchors=9, num_classes=20, compound_coef=0):
        ...
        self.bifpn = nn.Sequential(*[BiFPN(self.num_channels) for _ in range(1)])
...
if __name__ == '__main__':
    ...
    writer.add_graph(model, [torch.rand(4, 3, 512, 512), torch.ones(4,3,5)])
