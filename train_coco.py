        ...
        if epoch % opt.test_interval == 0:
            ...
            for te_iter, te_batch in enumerate(test_generator):
                ...
                with torch.no_grad():
                    ...
                    # print (te_loc_loss, te_focal_loss, num_te_pos_boxes)
            ...
            if loss_ls + opt.es_min_delta < best_loss:
                ...
                #     torch.onnx.export(model.module, dummy_input, os.path.join(opt.saved_path, "signatrix_detector.onnx"),
                #                       verbose=False)
                # else:
                #     torch.onnx.export(model, dummy_input, os.path.join(opt.saved_path, "signatrix_detector.onnx"),
                #                       verbose=False)
            # Early stopping
